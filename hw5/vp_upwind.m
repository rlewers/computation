...
Vab(1,:) = (r*param.ret.*grid.a(1,:) + w.*param.e).^(-1); %state constraint boundary condition    


%consumption and savings with forward difference
cf = max(Vaf,1e-08).^(-1);
sf = r*param.ret.*grid.a+w.*param.e-cf;

%consumption and savings with backward difference
cb = max(Vab,1e-08).^(-1);
sb = r*param.ret.*grid.a+w.*param.e-cb;
%consumption and derivative of value function at steady state
c0 = w*param.e + r*param.ret.*grid.a ;
Va0 = c0.^(-1);
